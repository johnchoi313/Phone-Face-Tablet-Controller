...
namespace SimpleInputNamespace {
	public class Joystick : MonoBehaviour, ISimpleInputDraggable {
		...
		public float GetX() { return xAxis.value; }
		public float GetY() { return yAxis.value; }
		...
		private void Awake() {
			...
			if( bgImage != null ) { background = bgImage; background.raycastTarget = false; }

			if( isDynamicJoystick ) { opacity = 0f; thumb.raycastTarget = false; OnUpdate(); }
			else { thumb.raycastTarget = true; }
		...
		private void Start() {
			...
			if( !isDynamicJoystick ) { eventReceiver = thumbTR.gameObject.AddComponent<SimpleInputDragListener>(); } 
		...
		public void OnDrag( PointerEventData eventData ) {
			...
			if( movementAxes == MovementAxes.X ) { direction.y = 0f; }
			else if( movementAxes == MovementAxes.Y ) { direction.x = 0f; }

			if( direction.sqrMagnitude > movementAreaRadiusSqr ) { direction = direction.normalized * movementAreaRadius; }
		...
		private void OnUpdate() {
			if( !isDynamicJoystick ) { return; }

			if( joystickHeld ) { opacity = Mathf.Min( 1f, opacity + Time.unscaledDeltaTime * 4f ); }
			else { opacity = Mathf.Max( 0f, opacity - Time.unscaledDeltaTime * 4f ); }
